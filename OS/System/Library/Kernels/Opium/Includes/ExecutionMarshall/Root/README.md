...
>Информация ниже не обновлялась со времён начала перевода на C++ и поэтому в основном верна только для прототипа на NW.JS. Актуальная версия шагнула далеко вперёд по части базы: парсер стал декларативным semi-GLL; система типов получила рабочий каркас и формально разделилась на абстрактные (типы) и конкретные (значения); алгоритмы разрешения имён были полностью переписаны; появился IPC-сервер. Между тем, это не значит, что язык уже можно как-либо использовать, или что там есть на что смотреть - для этого лучше обратиться к старому прототипу, где были реализованы рабочие функции и композиты.
...
Наблюдатели не объявляются самостоятельно, а закрепляются за другими мемберами или за всем композитом вцелом. Они используются для изменения обычного поведения при записи и чтении в них.
...
#### Вызовы, области видимости и передачи управления
...
- Передача управления: значение (примитивное значение), тип (строка). Оба свойства могут быть незаполнены.
